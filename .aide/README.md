...
| `local.toml.example` | The shape of the **personal, per-machine** config (`local.toml`, gitignored): the filesystem paths a committed file may not hold |
...
- **Shared (committed):** `.aide/` (minus `local.toml`), `aide.toml`,
...
there, and what a non-interactive session cannot do (ask a question, above all —
which is why `aide.toml`'s `[loop] clarify = "assume"` belongs in place before
such a run).
