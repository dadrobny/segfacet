...
- **Left open:** `catalogue._scan_synth_rule_mode_map` derives the "corpus"
  ...
  scan.
- **B1: implementation (builder, 2026-09-24).**
  - `FusePerturbation(bridged=True)` walks columns on `np.moveaxis` of a
    private copy. The low/high label of each column is chosen by the pair's
    mean index on `si_axis`. The span is `(max index of the low label, min
    index of the high label)`, exclusive, and only background voxels in it
    are filled. The neighbour relabel and the caudal renumbering both read
    their masks from the original input array.
  - The refusal for a neighbour that is not the next-higher present label
    applies only when `bridged=True`. The unbridged branch returns the same
    array, rule ids, labels, verdict and `detail` string as before.
  - The `Expectation` call takes `rule_ids`/`offending`/`verdict` from local
    names set per branch. The literal-only scan now reads
    `{'fragmentation': (1, 3, 4), 'coverage': (6,), ...}` (A6), and
    `rule_declaration_conflicts()` is `()`.
  - **Change outside A2's letter: `_new_image` passes `dtype=data.dtype`.**
    AC7 feeds the operator `loaded_seg_image(...)`, whose data is `int64`.
    nibabel refuses `Nifti1Image(int64_array, affine)` with no dtype, so AC7
    failed. The helper is shared by every operator in the module. For the
    `uint16` base the header is unchanged: a third corpus regeneration after
    this change is byte-identical to the committed tree.
- **B2: re-measured values (builder, 2026-09-24).** All match A3 and A6:
  - bridge: 6 262 voxels filled over 682 columns (the columns with at least
    one filled voxel);
  - label 22: one component of 45 043 voxels (45 043 mm³); labels present
    [20, 21, 22, 23];
  - `segfacet run --no-reference` on the committed fixture: verdict `pass`,
    no findings;
  - cohort: tp 10, fp 0, tn 3, fn 1; sensitivity 10/11; FPR 0.0; mode 2 is
    (0, None); per-mode `n_cases` sum is 11; the other modes are unchanged;
  - catalogue: exactly A6's eight `failure_modes` edits (six component paths
    [1, 2, 3, 4] → [1, 3, 4], two relationship paths [2, 6] → [6]);
    `mode2_count` 15 → 7; entry count 140, mode 1 count 14, mode 16 count 2
    and the `mode_evidence` distribution are unchanged.
- **B3: regeneration (builder, 2026-09-24).** Each generator was run twice
  into temp, and the two runs were byte-identical. The corpus differed from
  the committed tree only in `tests/corpus/manifest.json` (the
  `fuse_adjacent` entry) and `fixtures/fuse_adjacent_seg.nii.gz`. The
  failure_modes, traceability and feature_catalogue `.json`/`.md` documents
  were copied in. `golden_evidence` regenerated byte-identical to the
  committed `docs/aide/golden_evidence.generated.json`, as R4 predicted.
- **B4: test reconciliation (builder, 2026-09-24).**
  - (a) `test_040`: `test_fuse_adjacent_case_records_mode_2_with_both_co_detected_rules`
    → `test_fuse_adjacent_case_records_mode_2_bridged_and_fires_nothing`
    (R2). `expected_rule_ids` `["coverage", "fragmentation"]` → `[]`,
    `expected_labels == []` added, `expected_verdict` `"flagged-for-review"` →
    `"pass"`, and the subset check → `list(findings) == []` plus
    `Severity.PASS`.
  - (a) `test_057`: `_PIPELINE_DETECTABLE_MODES` `(1, 2, 3, 4, 6, 9)` →
    `(1, 3, 4, 6, 9)`.
    `test_overall_corpus_sensitivity_is_nine_of_ten_not_over_claimed`:
    `11.0 / 12.0` → `10.0 / 11.0`.
  - (a) `test_120::test_ac24_corpus_pipeline_detection_is_nine_of_ten`:
    `11.0 / 12.0` → `10.0 / 11.0`; the `2: 1.0` entry is removed from
    `expected_sensitivity`; `sum(n_cases)` `12` → `11`.
  - (a) `test_103::_RULE_MODE_MAP`: `"fragmentation"` `(1, 2, 3, 4)` →
    `(1, 3, 4)`, `"coverage"` `(2, 6)` → `(6,)`.
  - (a) `test_136::test_ac4_corroborated_modes_are_covered_by_the_measured_corpus_map`:
    `expected_co_detections`
    `{("coverage", 2), ("fragmentation", 2), ("mislabel", 1)}` →
    `{("mislabel", 1)}`.
  - (a) `test_137::test_adv_measured_artifact_movement_counts_from_spec`:
    `mode2_count` `15` → `7`, with a "Re-measured (item 176)" paragraph.
  - (b) `test_167::test_fuse_adjacent_stays_silent`:
    `component_count > 1` → `== 1`, with a re-worded message.
    `stray_contact_area_mm2 == 0.0` is kept.
  - (b) `test_125::test_ac15_agrees_with_test_057_pipeline_detectable_modes`
    and `test_135::test_ac25_agrees_with_test_057_pipeline_detectable_modes`:
    the comparison set is built inline from pipeline-path failure cases with
    a non-empty `expected_rule_ids`, now `{1, 3, 4, 6, 9}`. The shared
    helper is not edited.
  - No other test was edited, renamed, skipped or loosened. Full suite:
    9240 passed, 72 skipped (Docker, CuPy, VerSe, SPINEPS and PyRadiomics
    gated), 0 failed.
