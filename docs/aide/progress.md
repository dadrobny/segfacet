...
| Stage 31 D1 — `vision.md` §6 re-issued as v4: the maintainer reads the draft (PR #77, branch `docs/vision-v4-section-6`) and either accepts it or names the passages to change. What it asks: §6 as principles plus a pointer to `segfacet.failure_modes.SPECIFICATION` with no numbered list, the five observability classes, the FOV-truncation condition as a first-class concept, `validated` as the item-150 sign-off defined it, `scope` / `parent` / one defect per mode, the mode → rule direction scored, and the evidence-rungs example corrected to a single rank descent | stage 31 | ⏳ Awaiting | Raised 2026-09-16. The roadmap says this gate blocks D2 only; no Stage 31 item exists yet, so the reach is `stage 31` and is to be narrowed to D2's item number when queue-021 is planned. Approving is a decision on the content and is independent of merging: the PR stays a draft until D2 has re-pointed the nine seed-list reader tests (`test_137` AC17, `test_138` AC9, `test_144` AC16 ×2, `test_145` AC3, `test_147` AC4/AC5/adv-AC5, `test_151` AC27) on the queue branch, then the two merge back to back so `main` is never red. |
